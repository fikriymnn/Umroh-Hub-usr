...
const CardPackage = ({ packageList = [], capacity, booked }: any) => {
    ...
    return (
        <Link to="/UmrohPackage/:id" className='w-full relative h-[200px] shadow-[-1px_1px_8px] shadow-white'>
            <img src={packageList.image} alt="paket" className='w-full h-full object-cover' />
            <div className="w-full h-full bg-gradient-to-b from-black/0 to-black py-3 px-5 flex flex-col space-y-3 absolute top-0">
                ...
                <div className="flex space-x-2">
                    <img src={packageList.hotelIcon2} alt="background" className="w-[30px] h-[30px]" />
                    <div className="flex-col w-full">
                        <div className="flex w-full">
                            <h6 className='text-white text-[12px] font-semibold'>{packageList.hotelName} :
                                <span className='text-yellow-300 mx-[7px]'>{renderStars(packageList.hotelRating)}</span>
                                (Quad Room)
                            </h6>
                        </div>
                        <h1 className="text-[#5CE9FF] text-[10px] font-semibold">{packageList.hotelDistance}</h1>
                    </div>
                </div>
                <div className="w-full flex justify-between">
                    <div className="flex space-x-2">
                        <img src={packageList.planeIcon} alt="background" className="w-[30px] h-[30px]" />
                        <div className="flex-col w-full">
                            <div className="flex w-full">
                                <h6 className='text-white text-[12px] capitalize font-semibold'>{packageList.airline}
                                    <span className='text-yellow-300 mx-[7px]'>{renderStars(packageList.airlineRating)}</span>
                                </h6>
                            </div>
                            <h1 className="text-[#5CE9FF] text-[10px] font-semibold">{packageList.route}</h1>
                        </div>
                    </div>
